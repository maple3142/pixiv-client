// Customized Type definitions for got 9.2
...
declare module 'got' {
	...
	class RequestError extends StdError {
		name: 'RequestError'
	}

	class ReadError extends StdError {
		name: 'ReadError'
	}

	class ParseError extends StdError {
		name: 'ParseError'
		statusCode: number
	...
	class MaxRedirectsError extends StdError {
	...
	class UnsupportedProtocolError extends StdError {
		name: 'UnsupportedProtocolError'
	}

	class CancelError extends StdError {
		name: 'CancelError'
	}

	class TimeoutError extends StdError {
		name: 'TimeoutError'
	}
	...
	type fn = got.GotFn &
		Record<'get' | 'post' | 'put' | 'patch' | 'head' | 'delete', got.GotFn> & {
			...
			extend(opt: got.GotOptions<string>): fn
		}
	const got: fn

	interface InternalRequestOptions extends https.RequestOptions {
		// Re options with `any` type for allow specify types incompatible with http.RequestOptions.
		timeout?: any
		agent?: any
	}
